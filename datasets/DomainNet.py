...
class DomainNet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # return img, label# source pretraining
        return img, label, index# target adaptation

    def __len__(self):
        return len(self.data_paths)


def get_domainnet_dloader(base_path, domain_name, batch_size, num_workers, Isshuffle=True):
    # dataset_path = path.join(base_path, 'dataset', 'DomainNet')
    dataset_path = path.join(base_path, 'data', 'DomainNet')
    ...
    train_dloader = DataLoader(train_dataset, batch_size=batch_size, num_workers=num_workers, pin_memory=False,
                            shuffle=Isshuffle)#pin_memory=True
    test_dloader = DataLoader(test_dataset, batch_size=batch_size, num_workers=num_workers, pin_memory=False,
                            shuffle=Isshuffle)#pin_memory=True
