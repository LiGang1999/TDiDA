...
def train_target(args):
    ...
    cnt = 0
    gap = torch.zeros(len(args.src),)
    for _, inputs_weight_ in enumerate(dset_loaders["target"]):
        cnt = cnt + 1
        for i in range(len(args.src)):
            with torch.no_grad():
                inputs_weight = inputs_weight_[0].cuda()
                features_weight = netB_list[i](netF_list[i](inputs_weight))
                outputs_weight = netC_list[i](features_weight)
                softmax_weight = nn.Softmax(dim=1)(outputs_weight)
                max_weight, pos = torch.max(softmax_weight, dim=1)
                for j in range(inputs_weight.size()[0]):
                    softmax_weight[j, pos[j]] = 0.0
                submax_weight = torch.max(softmax_weight, dim=1)[0]
                gap[i] = gap[i] + torch.mean(max_weight - submax_weight)
    print('gap: ', gap / cnt)
    gap = gap * 5. / cnt
    w = torch.softmax(gap, dim=0)
    ###

    netG_list = [network.scalar(w[i]).cuda() for i in range(len(args.src))]
    for _, v in netG_list[i].named_parameters():
        param_group += [{'params':v, 'lr':args.lr * 0.0}]# Fix domain weight
    ...
    while iter_num < max_iter:
        try:
            ...
            inputs_test, labels_test, tar_idx = iter_test.__next__()
        except:
        ...
        for i in range(len(args.src)):
            ...
            weights_test = netG_list[i](features_test)# torch.Tensor([[0], [1], ..., [bs - 1]])
            outputs_all[i] = outputs_test
            weights_all[:, i] = weights_test.squeeze()# torch.Tensor([[0] * n, [1] * n, ..., [bs - 1] * n])

        z = torch.sum(weights_all, dim=1)
        z = z + 1e-16

        weights_all = torch.transpose(torch.transpose(weights_all, 0, 1) / z, 0, 1)# torch.Tensor([[0] * n, [1] * n, ..., [bs - 1] * n])
        outputs_all = torch.transpose(outputs_all, 0, 1)# bs * src * cls
        ...
        if args.cls_par > 0:
            ...
            with torch.no_grad():
                for i in range(len(args.src)):
                    src_fea = all_feas[i]
                    dd = torch.cdist(src_fea[tar_idx, :].float(), initc[i].float(), p=2)
                    dd_soft = torch.softmax(dd * -1., dim=1)

                    dd_max, pos = torch.max(dd_soft, dim=1)
                    for j in range(dd_soft.size()[0]):
                        dd_soft[j, pos[j]] = 0.0
                    dd_submax = torch.max(dd_soft, dim=1)[0]
                    dd_gap = dd_max - dd_submax
                    dd_soft = dd_soft * 0.0
                    for j in range(inputs_test.size()[0]):
                        dd_soft[j, pos[j]] = dd_gap[j]

                    if i==0:
                        dd_soft_sum = dd_soft * w[i]
                    else:
                        dd_soft_sum = dd_soft_sum + dd_soft * w[i]
                pred_label = dd_soft_sum.argmax(dim=1)
                pred_label = pred_label.int()
                pred = pred_label.long()
            ###
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='TDiDA')
    ...
    parser.add_argument('--net', type=str, default='resnet50', help="alexnet, vgg16, resnet50, res101, vit")
    ...
    parser.add_argument('--bottleneck', type=int, default=256)#2048
...
# CUDA_VISIBLE_DEVICES=1 python image_target_Multi_Src.py --da uda --dset office-home --t 1 --output_src ckps/source/ --output ckps/target_Multi_Src/ > log_t1_office-home.txt

# CUDA_VISIBLE_DEVICES=2 python image_target_Multi_Src.py --da uda --dset office-home --t 2 --output_src ckps/source/ --output ckps/target_Multi_Src/ > log_t2_office-home.txt

# CUDA_VISIBLE_DEVICES=3 python image_target_Multi_Src.py --da uda --dset office-home --t 3 --output_src ckps/source/ --output ckps/target_Multi_Src/ > log_t3_office-home.txt
